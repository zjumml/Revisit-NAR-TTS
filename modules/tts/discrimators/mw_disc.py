...
class Discriminator2DFactory(nn.Module):
    def __init__(self, time_length, freq_length=80, kernel=3, c_in=1, hidden_size=128,
        ...
        def discriminator_block(in_filters, out_filters, first=False):
            ...
            Output:(B, out, H,  W)
            """
            conv = nn.Conv2d(in_filters, out_filters, kernel, 2, padding)
            if norm_type == 'sn':
        ...
        self.ds_size = ds_size = (time_length // 2 ** 3, (freq_length + 7) // 2 ** 3)
        if reduction != 'none':
...
class MultiWindowDiscriminator(nn.Module):
    def __init__(self, time_lengths, cond_size=0, freq_length=80, kernel=3,
...
class Discriminator(nn.Module):
    def __init__(self, time_lengths=[32, 64, 128], freq_length=80, cond_size=0, kernel=3, c_in=1,
    ...
    def forward(self, x, cond=None, start_frames=None):
        ...
        ret = {'y_c': None, 'y': None}
        if self.uncond_disc:
            ret['y'], start_frames, ret['h'] = self.discriminator(
                x, x_len, start_frames_wins=start_frames)
        if self.cond_size > 0 and cond is not None:
            ret['y_c'], start_frames, ret['h_c'] = self.cond_disc(
                x, x_len, cond, start_frames_wins=start_frames)
        ret['start_frames'] = start_frames
        return ret
